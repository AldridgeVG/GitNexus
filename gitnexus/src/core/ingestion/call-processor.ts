...
export const processCalls = async (
  ...
  for (let i = 0; i < prepared.length; i++) {
    ...
    matches.forEach((match) => {
      ...
      // The worker path sets isInherited for these; the sequential path must do the same.
      if (language === SupportedLanguages.Pascal && callNode.type === 'inherited' && heritageMap) {
        let calledName = captureMap['call.name']?.text;
        const enclosingFuncId = findEnclosingFunction(callNode, file.path, ctx, provider);
        const sourceId = enclosingFuncId || generateId('File', file.path);
        if (!calledName && enclosingFuncId) {
          // Bare 'inherited;' — infer method name from enclosing function
          const lastColon = enclosingFuncId.lastIndexOf(':');
          const segment = lastColon >= 0 ? enclosingFuncId.slice(lastColon + 1) : '';
          const dotIdx = segment.lastIndexOf('.');
          calledName = dotIdx >= 0 ? segment.slice(dotIdx + 1) : segment;
        ...
        if (calledName && !provider.isBuiltInName(calledName)) {
          const resolved = resolveInheritedCall(
            { calledName, sourceId },
            file.path,
            ctx,
            heritageMap,
          ...
          if (resolved) {
            graph.addRelationship({
              id: generateId('CALLS', `${sourceId}:${calledName}->${resolved.nodeId}`),
              sourceId,
              targetId: resolved.nodeId,
              type: 'CALLS',
              confidence: resolved.confidence,
