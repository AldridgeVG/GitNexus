...
; Procedure/function implementations (in implementation section)
; Handles both standalone functions (identifier name) and class methods (genericDot qualified name)
(defProc
  header: (declProc
...
; Constructor/method access without parentheses in assignment RHS: Event := TClass.Create;
; The (#match? @type.name "^T[A-Z]") predicate filters to Pascal type names
; (e.g. TAppStatusEvent.Create) and avoids false positives from variable field
; accesses like FsdcContext.gMID.
(assignment
  (kAssign)
  .
  (exprDot
